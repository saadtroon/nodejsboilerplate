...
async function stichLayers(layerNum, shoeTypes, assignedNFT, baseImage) {
    ...
    if (layerNum == 0) {
        ...
        finalImage = await ShoefyModel.find(query).then( async function(shoefy) {
            ...
            var query4 = { layerNum: 5, shoeType: shoeTypes,categoryName:shoefy[0].categoryName.toUpperCase(), traitType:"BACK",imageName: shoefy[0].back}
            var query5 = { layerNum: 6, shoeType: shoeTypes,categoryName:shoefy[0].categoryName.toUpperCase(), imageName: shoefy[0].shoeSideColourGradient}
            ...
            let fourthLayer = await ImagesModel.find(query4).then(async function(images) {
                return images[0].image;
            }, function(err) {
                console.log(err);
            });

            let fifthLayer = await ImagesModel.find(query5).then(async function(images) {
                return images[0].image;
            }, function(err) {
                console.log(err);
            });
            ...
            baseImage = await stichImages(baseImage, fourthLayer);
            baseImage = await stichImages(baseImage, fifthLayer);
