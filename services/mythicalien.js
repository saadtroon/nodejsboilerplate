...
async function stichService(farm) {
        ...
        if (glayerNumbers == 13) {farm.mintStatus = "Completed"}
        return farm;
    }
...
async function stichLayers(layerNum, shoeTypes, assignedNFT, baseImage) {
    ...
    if (layerNum == 12) {
        ...
        finalImage = await ShoefyModel.find(query).then( async function(shoefy) {
            ...
            query = { layerNum: 13, shoeType: shoeTypes, categoryName:shoefy[0].categoryName.toUpperCase(), imageName:shoefy[0].assetLaser}
